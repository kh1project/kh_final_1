	...
	<ul>
		<c:choose>
			<c:when test="${fn:endsWith(path, 'reserve.jsp') }">
				<li class="choice-movie active" onclick="location.href='<%=request.getContextPath() %>/reserve?location='">영화</li>
			...
			<c:when test="${fn:endsWith(path, 'schedule.jsp') }">
				<li class="choice-movie" onclick="location.href='<%=request.getContextPath() %>/reserve?location='">영화</li>
